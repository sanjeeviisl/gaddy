jQuery(document).ready(function($) {
    ...
    function getAllRoutesForMap() {
        ...
        $.ajax({
            url: 'mapview/getallroutesformap.php',
            type: 'GET',
            dataType: 'json',
    ...
    function getRouteForMap() { 
        if (hasMap()) {
           ...
           var url = 'mapview/getrouteformap.php' + $('#routeSelect').val();
...
//	getRouteForMapUpdated
    function getRouteForMapUpdated() { 
        if (hasMap()) {
            // console.log($("#routeSelect").prop("selectedIndex"));
...
//loadGPSLocationsUpdated
    function loadGPSLocationsUpdated(json) {
        // console.log(JSON.stringify(json));
				...
				$(json.locations).each(function(key, value){
                    ...
                    finalLocation = true;
                    var marker = createMarker_updated(
                        latitude,
                        longitude,
                        $(this).attr('speed'),
                        $(this).attr('direction'),
    ...
    function createMarker_updated(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
                          userName, sessionID, accuracy, extraInfo, map, finalLocation) {
        var iconUrl;

        if (finalLocation) {
            iconUrl = 'mapview/images/coolred_small.png';
        } else {
            iconUrl = 'mapview/images/coolgreen2_small.png';
        }

        var markerIcon = new L.Icon({
                iconUrl:      iconUrl,
                shadowUrl:    'mapview/images/coolshadow_small.png',
                iconSize:     [12, 20],
                shadowSize:   [22, 20],
                iconAnchor:   [6, 20],
                shadowAnchor: [6, 20],
    ...
    function createMarker(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
        ...
        if (viewingAllRoutes) {
            ...
            gpstrackerMarker.on("click", function() {        
                var url = 'mapview/getrouteformap.php?sessionid=' + sessionID;
    ...
    function restartInterval() {
        ...
        if (viewingAllRoutes) {
            intervalID = setInterval(getAllRoutesForMap, 6 * 1000); // one minute 
        } else {
            intervalID = setInterval(getRouteForMap, 6 * 1000);          
        }        
		
	//	intervalID = setInterval(getRouteForMapUpdated, 3 * 1000); // one minute 
    ...
    function deleteRoute() {
        if (hasMap()) {
            ...
            if (answer){
                var url = 'mapview/deleteroute.php' + $('#routeSelect').val();
